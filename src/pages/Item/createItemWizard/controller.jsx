export default function CreateItemWizardController (view) {
    var context = this;
    context.view = view;
    ...
    context.performDeploy = async function performDeploy() {
        ...
        var transaction = await window.blockchainCall(state.selectedToken.contract.methods["mint(uint256,string,string,string,bool)"], valueDecimals, state.itemName, state.itemSymbol, metadataLink, state.itemMintable);
        ...
        for(var event of events) {
            ...
            try {
                objectId = window.web3.eth.abi.decodeParameter("uint256", event.raw.topics[1]);
            } catch(e) {
                objectId = window.web3.eth.abi.decodeParameters(["uint256", "address", "uint256"], event.raw.data)[0];
            }
            break;
        }
        ...
        delete window.loadCollections;
        context.view.setState({collectionAndItem});
