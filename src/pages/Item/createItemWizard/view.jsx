...
import CreateOrEditTraitTypes from './createOrEditTraitTypes'
import InnerLoader from './innerLoader'
import FullLoader from './fullLoader'
import CreateItemWizardController from './controller'
...
export default createReactClass({
        ...
        return (<section className="createITEM">
            <section className="FormCreate">
                ...
                <section className="FormCreateThing">
                    ...
                    {metadataType === 'custom' && (!this.state || this.state.performing !== 'deploy') && <a href="javascript:;" ref={ref => ref && (ref.view = _this)} data-action="deploy" className="SuperActionBTN" onClick={window.perform}>DEPLOY</a>}
    ...
    renderStep3() {
        ...
        return (<section className="createITEM">
            <section className="FormCreate">
                ...
                <section className="FormCreateThing">
                    <a className={"SuperActionBTN SuperActionBTNB" + (this.state && this.state.performing ? " disabled" : "")} href="javascript:;" onClick={this.back}>BACK</a>
                    {(!this.state || this.state.performing !== 'deploy') && <a href="javascript:;" ref={ref => ref && (ref.view = _this)} data-action="deploy" className="SuperActionBTN" onClick={window.perform}>DEPLOY</a>}
                    {this.state && this.state.performing === 'deploy' && <InnerLoader />}
                    {this.state && this.state.loadingMessage && <span>{this.state.loadingMessage}</span>}
                </section>
            </section>
    ...
    componentDidMount() {
        this.controller = new CreateItemWizardController(this);
        var _this = this;
        var callback = function() {
            if(!window.ethItemOrchestrator) {
                return setTimeout(callback);
            }
            _this.props.collectionAddress && _this.controller.onTokenAddressChange(_this.props.collectionAddress).then(() => _this.setState({step : 1}));
        }
        return setTimeout(callback);
    },
    render() {
        var _this = this;
        var state = this.getState();
        if(state.collectionAndItem) {
            return (<div>
                <h3>Congratulations!</h3>
                <p>{state.collectionAndItem.name} has been deployed!</p>
                <div>
                    <a href="javascript:;" onClick={() => _this.setState({step : 1, collectionAndItem : null})}>OK</a>
                </div>
            </div>);
        }
